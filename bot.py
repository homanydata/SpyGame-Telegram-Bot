...
class SpyGameBot:
    ...
    def show_results(self, session):
        self.bot.send_message(chat_id=session.chat_id, text=Messages.show_results(spy=session.spy, word=session.word))
    ...
    def start_timer(self, session:GameSession):
        ...
        timer = threading.Timer(180, timer_callback, session=session)  # 180 seconds = 3 minutes
        timer.start()

    def send_private_messages(self, session:GameSession):
        for player in session.players:
            if player != session.spy:
                self.bot.send_message(chat_id=player.id, text=Messages.send_word(word=session.word))
            else:
